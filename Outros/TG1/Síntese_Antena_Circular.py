...
if test:
    ...
    def Probe(k01, k10, Kef, Dtheta, Dphi):
        ...
        phip = theta1c + np.arccos(R(np.cos(thetap),L10,M10,theta1c) / R(np.cos(thetap),L01,M01,theta1c) * np.abs(V)) / M01

        print(phip[49]-theta1c, phip[50]-np.pi/2, phip[51], np.pi-phip[51]-phip[49], '\n\n')
        u = np.cos(thetap) # + np.cos(thetap)[::-1]
        print(u[49], u[50], u[49]+u[51], '\n\n')
        # exit()
        return phip, thetap
    ...
    def plotRA(k01, k10, tgef, Dtheta, Dphi, thetap, phip):
        freqs = np.linspace(1.53e9, 1.6e9, 61)
        ...
        # print(RAfdB[36:46]-3, '\n')
        # print(freqs[37], freqs[43])
        # print((freqs[43]-freqs[37])*1e-6, 'MHz\n')
    ...
    def plotRAangle(k01, k10, tgef, Dtheta, Dphi, phip):
        thetap = np.linspace(0, 2*np.pi, 181)
    ...
    def imZin(p, Analysis = 0):
        ...
        Ph = np.concatenate((Ph[:51], np.pi-Ph[:50][::-1]))
        # print(len(Ph))
        # print('TH', Th - (np.pi-Th)[::-1])
        # print(Ph - (np.pi-Ph)[::-1])
        ...
        if  Analysis:
            ...
            plotRAangle(k01, k10, tgef, Dtheta, Dphi, Phtheta(thetap))
    ...
    if project:
        ...
        Dphi, Dtheta, thetap, phip, Zin = imZin(0.58173828125, Analysis = 1) # Old
        # Dphi, Dtheta, thetap, phip, Zin = imZin(0.5766601562500001, Analysis = 1)
