...
if test:
    ...
    def imZin(p, Analysis = 0):
        ...
        # print(k01, Dtheta/dtr, Dphi/dtr, Dtheta/dtr-2*deltaPhi/dtr, Dphi/dtr-2*deltaPhi/dtr, tgef)
        ...
        while epsilon > tol:
            ...
            # print(S(kl, Dtheta, Dphi), np.angle(S(kl, Dtheta, Dphi))/dtr, phaseK/dtr, k10, k01)
            ...
            # print(Dphi/dtr-2*deltaPhi/dtr, Dtheta/dtr-2*deltaPhi/dtr, tgef, epsilon)

        if not Analysis:
            print('k01 e k10 final: ', k01, k10)
            print('Frequências finais (MHz): ', k01/(2*np.pi*np.sqrt(u0*es))/1e6, k10/(2*np.pi*np.sqrt(u0*es))/1e6)
        ...
        # print(thetap/dtr, Phtheta(thetap)/dtr)
