...
print('v: ', rootsLambda[L][M],  M * np.pi / (phi2c - phi1c))
...
print(Integ(np.cos(theta1c),L,M)-Integ(np.cos(theta2c),L,M))

print('Erro: ', np.abs(Integ(np.cos(theta1c),L,M)-Integ(np.cos(theta2c),L,M)-Ilm)/Ilm)
