...
if test:
    ...
    def plotZ(k01, k10, thetap, phip, tgef, Dtheta, Dphi):    
        freqs = np.linspace(1.4e9, 1.8e9, 641)

        ZinP = Z(freqs, k01, k10, thetap, phip, np.exp(1.5)*df/(2*a_bar*np.sin(thetap)), tgef, Dtheta, Dphi)
    ...
    def imZin(p, Analysis = 0):
        ...
        if  Analysis:
            plotZ(k01, k10, thetap, Phtheta(thetap), tgef, Dtheta, Dphi)
    ...
    if project:
        ...
        Dphi, Dtheta, thetap, phip, Zin = imZin(0.58173828125, Analysis = 1)

        theta1c, theta2c = np.pi/2 - Dtheta/2, np.pi/2 + Dtheta/2
        phi1c, phi2c = np.pi/2 - Dphi/2, np.pi/2 + Dphi/2

        def Franjas(angulo, angulo_c):
            F = np.log(b*angulo/(2*h))+1.41*er+1.77+h*(0.268*er+1.65)/(b*angulo)
            Exp = angulo*np.sqrt(1-(2*h*F)/(np.pi*b*angulo*er))-angulo_c
            print(Exp)
            return Exp
        
        plt.figure()
        plt.plot(np.linspace(0, 2*np.pi, 641)/dtr, Franjas(np.linspace(0, 2*np.pi, 641), np.pi/2))
        plt.grid(True)
        plt.show()


        print('Franja antes: ', deltaPhi)
        rootF = root_scalar(partial(Franjas, angulo_c = np.pi/2), bracket=[1e-3, 0.1], method='bisect')
        deltaPhi = deltatheta1 = deltatheta2 = rootF.root
        print('Franja depois: ', deltaPhi)
