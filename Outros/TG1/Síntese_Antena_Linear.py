...
if test:
    ...
    def Z(f, k10, thetap, phip, Dtheta, Dphi):
        tgef = tgefTot(k10, Dtheta, Dphi)
        ...
        return RLC(f, k10, L10, M10, thetap, phip, Dphip, tgef, Dtheta, Dphi) + 1j*Xp
    ...
    def tgefTot(klm, Dtheta, Dphi, mode = '10'):
        ...
        if mode == '10':
            ...
            Q10 = (np.pi / 12) * klm * np.sqrt(er) * (b**3 - a**3) * I10 * Dphi / (np.abs(R(np.cos(theta1c),L10,M10,theta1c))**2 * S10)
            ...
            Q01 = (np.pi / 96) * klm * np.sqrt(er) * (b**3 - a**3) * I01 * Dphi / (np.abs(R(np.cos((theta1c+theta2c)/2),L01,M01,theta1c))**2 * S01)
            return tgdel + 1/Qc + 1/Q01
        
    def plotZ(k10, thetap, phip, Dtheta, Dphi):    
        freqs10 = np.linspace(1.4e9, 1.8e9, 641)

        ZinP = Z(freqs10, k10, thetap, phip, Dtheta, Dphi)

        plt.figure()
        plt.plot(freqs10 / 1e9, np.real(ZinP), label='re(Z)')
        plt.plot(freqs10 / 1e9, np.imag(ZinP), label='im(Z)')
        plt.plot(freqs10 / 1e9, [Z0] * len(freqs10), label='Z0')
        # plt.plot(flm_des / 1e9, Z0)
    ...
    # Início
    def LinProj(Analysis = 0):
        flm_new = flm_des
        k10 = 2*np.pi*flm_des*np.sqrt(u0*es)
        phip = np.pi/2
        ...
        while epsilon > tol:
            def Z50(theta):
                return np.real(Z(flm_des, k10, theta, phip, Dtheta, Dphi))-50
            root = root_scalar(Z50, bracket=[np.pi/2, np.pi/2+Dtheta/2-0.02-h/a], method='bisect')
            thetap = np.array(root.root)

            # plotZ(k10, thetap, phip, Dtheta, Dphi)

            def Z0(f):
                return np.imag(Z(f, k10, thetap, phip, Dtheta, Dphi))
            ...
            lbd = root_find(0, theta1c, theta2c, phi1c, phi2c)[0][1]
            k10 = np.sqrt(lbd*(lbd+1))/a_bar

            Zin = Z(flm_des, k10, thetap, phip, Dtheta, Dphi)
        ...
        if  Analysis:
            ...
            Zinth = Z(flm_des, k10, Th, Ph, np.exp(1.5)*df/(2*a_bar*np.sin(Th)), Dtheta, Dphi)
    ...
    Dphi, Dtheta, thetap, phip, Zin = LinProj(Analysis = 0)
