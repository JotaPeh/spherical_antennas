...
if test:
    ...
    def imZin(p, Analysis = 0, Geometry = 0):
        ...
        Zinth = Z(flm_des, k01, k10, Th, Ph, np.exp(1.5)*df/(2*a*np.sin(Th)), tgef, Dtheta, Dphi)

        def Z50(theta):
            return np.real(Z(flm_des, k01, k10, theta, Phtheta(theta), np.exp(1.5)*df/(2*a*np.sin(theta)), tgef, Dtheta, Dphi))-50
        ...
        if Analysis:
            plt.figure()
            plt.plot(Th/dtr, np.imag(Zinth)-np.imag(Zinth[::-1]))
        ...
        Zin = Z(flm_des, k01, k10, thetap, Phtheta(thetap), np.exp(1.5)*df/(2*a*np.sin(thetap)), tgef, Dtheta, Dphi)
        # print('Zin = ', Zin)
    ...
    if project:
        ...
        # k01 e k10 final:  53.07323186153204 52.215840645686654
        # 34.173807104264725 34.53058757890866 84.09003549470647 85.34187755725492 (50.000000000815405+5.038826520475936e-06j)
        # Tempo de projeto:  521.5022225379944  s -> 8min42s
        # p =  0.5944724082946777
        Dphi, Dtheta, thetap, phip, Zin = imZin(1-0.58173828125, Analysis = 1, Geometry = 1)
